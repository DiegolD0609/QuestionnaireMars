...
function CoatingPart({ uniqueIdentifier }) {
 ...
 const localStorageExplainKey = `substanceCoatExplain_${uniqueIdentifier}`;
 const localStoragePresenceKey = `substanceCoatPresence_${uniqueIdentifier}`;
  const localStorageKey = `nameCoating01_${uniqueIdentifier}`;
  const localStorageSelectKey1 = `subCategoryCoating01_${uniqueIdentifier}`;
  const localStorageSelectKey2 = `fdaComplianceCoating01_${uniqueIdentifier}`;
  const localStorageTextInputKey = `otherCategoryCoating01_${uniqueIdentifier}`;
  const localStorageTextInputKey2 = `otherFdaCoating01_${uniqueIdentifier}`;
  const localStorageTextInputKey3 = `subsExpainlCoating01_${uniqueIdentifier}`;
  const localStorageTextInputKey4 = `gmpExplainCoating01_${uniqueIdentifier}`;
  const localStorageTextInputKey5= `supplierNameCoating01_${uniqueIdentifier}`;
  const localStorageToggleKey = `subspresenceCoating01_${uniqueIdentifier}`;
  const localStorageRadioKey = `gmpComplianceCoating01_${uniqueIdentifier}`;
  const localStorageResultVisible = `resultVisibleCoat01_${uniqueIdentifier}`;
  const localStorageSelectKey4 = `AbscenceSubsCoat01_${uniqueIdentifier}`;
 ...
 useEffect (() => {
  ...
  return (
    <><div className="fixed-header">
      <h1 className="header-title">Coating Questionnaire</h1>
        ...
        <div className="instructions">
          ...
          <label className="label">
            ...
            <select className="select" value={selectedOption1} onChange={handleSelectChange1}>
              ...
              <option value="epoxyPhenolicBased">Epoxy Phenolic Based</option>
              <option value="phenolicMelamineFormaldehyde">Phenolic - Melamine Formaldehyde</option>
              <option value="solventBased">Solvent Based</option>
              <option value="other">Other</option>
              <option value="uvEb">UV/EB</option>
              <option value="vinylicOrganasol">Vinylic Organasol</option>
            ...
            <div className="checkbox-group">
              ...
              <label className="checkbox-label">
                <input
                  type="checkbox"
                  value="175.320"
                  checked={selectedOption2.includes('175.320')}
                  onChange={handleSelectChange2} />
                175.320
              </label>
              <label className="checkbox-label">
                <input
                  type="checkbox"
                  value="175.230"
                  checked={selectedOption2.includes('175.230')}
                  onChange={handleSelectChange2} />
                175.230
              </label>
              <label className="checkbox-label">
